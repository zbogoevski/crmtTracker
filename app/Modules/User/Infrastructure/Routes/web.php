...
Route::middleware(['web', 'auth'])->group(function (): void {
    Route::get('/profile', [WebUserController::class, 'showProfile'])->name('profile.show');
    Route::put('/profile', [WebUserController::class, 'updateProfile'])->name('profile.update');
    Route::put('/profile/password', [WebUserController::class, 'updatePassword'])->name('profile.password.update');
    Route::post('/profile/2fa/setup', [WebUserController::class, 'setupTwoFactor'])->name('profile.2fa.setup');
    Route::delete('/profile/2fa/disable', [WebUserController::class, 'disableTwoFactor'])->name('profile.2fa.disable');
});

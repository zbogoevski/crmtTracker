...
@section('title', 'Edit Role - CRMTracker')
    ...
    <div class="p-8">
        <div class="max-w-4xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('web.roles.show', $role->id) }}" class="bg-slate-600 text-white px-4 py-2 rounded-lg hover:bg-slate-700 transition-colors">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                <form method="POST" action="{{ route('web.roles.update', $role->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        @error('guard_name')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-slate-500 text-xs mt-1">The guard name determines which authentication guard this role uses.</p>
                    </div>
